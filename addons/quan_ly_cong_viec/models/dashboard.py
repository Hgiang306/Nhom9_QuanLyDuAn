...
class Dashboard(models.Model):
    ...
    def _compute_tong_quan(self):
        for record in self:
            ...
            ngan_sach_records = self.env['ngan_sach_du_an'].search([])

            record.tong_ngan_sach = sum(ngan_sach_records.mapped('so_tien_du_kien'))

            record.tong_chi_phi = sum(ngan_sach_records.mapped('so_tien_da_chi'))
            
    def _compute_render_charts(self):
        for record in self:
            # Render cái template XML vào field HTML để hiển thị trong View Form
            record.render_charts = self.env['ir.qweb']._render('quan_ly_cong_viec.dashboard_du_an_template')
    

    @api.model
    def get_project_progress_stats(self):
        # Lấy tất cả dự án từ model du_an
        du_an_id = self.env['du_an'].search([])
        
        # Chuẩn bị dữ liệu cho Chart.js
        labels = [p.ten_du_an for p in du_an_id]
        data = [p.phan_tram_du_an for p in du_an_id]
        
        # Màu sắc ngẫu nhiên hoặc cố định cho các cột
        colors = ['#5b9bd5' for _ in du_an_id] 
    ...
    def get_dashboard_data(self):
        """Method to get or create dashboard data"""
        dashboard = self.search([], limit=1)
        if not dashboard:
            dashboard = self.create({})
        return dashboard.read()[0]

    @api.model
    def default_get(self, fields_list):
        """Override to ensure we always have a dashboard record"""
